...
/datum/discipline_power/melpominee/the_missing_voice/activate(atom/movable/target)
	...
	var/roll_pedik = secret_vampireroll(get_a_wits(owner)+max(get_a_expression(owner), get_a_intimidation(owner)), base_difficulty+difficulty_malus, owner)
	var/masked = FALSE
...
/datum/discipline_power/melpominee/phantom_speaker/pre_activation_checks(mob/living/target)
...
/datum/discipline_power/melpominee/madrigal/pre_activation_checks(mob/living/target)
...
/datum/discipline_power/melpominee/sirens_beckoning/pre_activation_checks(mob/living/target)
...
/datum/discipline_power/melpominee/shattering_crescendo/activate()
	...
	for(var/mob/living/carbon/human/listener in oviewers(7, owner))
		listener.Stun(2 SECONDS)
		listener.apply_damage(40, BRUTE, BODY_ZONE_HEAD)
