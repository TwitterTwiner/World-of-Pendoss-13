...
/datum/discipline_power/thaumaturgy_path_of_blood/proc/thaumaturgy_botch_effect()
	...
	switch(random_effect)
		...
		if(3)
			...
			owner.attributes.manipulation = max(owner.attributes.manipulation - 1, 1)
...
/datum/discipline_power/thaumaturgy_path_of_blood/a_taste_for_blood/activate(atom/target)
	...
	if(success_count > 2)
		...
		message += span_notice("This blood tastes like that of the [rand(7, blood_owner.generation)]\th generation.")
...
/datum/discipline_power/thaumaturgy_path_of_blood/blood_rage/activate(mob/living/carbon/human/target)
	...
	target.apply_status_effect(/datum/status_effect/blood_rage, success_count)
...
/datum/discipline_power/thaumaturgy_path_of_blood/blood_of_potency/activate()
	...
	for(var/i in 1 to points_to_spend)
		generation_choices += clamp((owner.generation - i), 7, 13)
	...
	chosen_generation = max(7, chosen_generation) //Lowest im gonna let you go is LOWEST_GENERATION_LIMIT bucko
