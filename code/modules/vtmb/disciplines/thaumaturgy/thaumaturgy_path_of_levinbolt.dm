/datum/discipline/thaumaturgy_path_of_levinbolt
	name = "Thaumaturgy: Path of the Levinbolt"
...
/datum/discipline_power/thaumaturgy_path_of_levinbolt
...
/datum/discipline_power/thaumaturgy_path_of_levinbolt/pre_activation_checks(atom/target)
	. = ..()
...
/datum/discipline_power/thaumaturgy_path_of_levinbolt/proc/levinbolt_target_click(mob/source, atom/target, params, include_radio_effects = FALSE)
	if(!active || !toggled)
		return
...
//SPARK - Level 1
/datum/discipline_power/thaumaturgy_path_of_levinbolt/one
	name = "Spark"
	...
	grouped_powers = list(
		/datum/discipline_power/thaumaturgy_path_of_levinbolt/three,
		/datum/discipline_power/thaumaturgy_path_of_levinbolt/five
	)
...
/datum/discipline_power/thaumaturgy_path_of_levinbolt/one/activate()
	. = ..()

	//signal for counterattack
	RegisterSignal(owner, COMSIG_PARENT_ATTACKBY, PROC_REF(spark_counter))
...
/datum/discipline_power/thaumaturgy_path_of_levinbolt/one/deactivate()
	. = ..()
...
/datum/discipline_power/thaumaturgy_path_of_levinbolt/one/proc/spark_counter(mob/source, obj/item/weapon, mob/living/attacker)
	SIGNAL_HANDLER
...
//ILLUMINATE - Level 2
/datum/discipline_power/thaumaturgy_path_of_levinbolt/two
	name = "Illuminate"
...
/datum/discipline_power/thaumaturgy_path_of_levinbolt/two/activate(mob/living/target)
	. = ..()
...
/datum/discipline_power/thaumaturgy_path_of_levinbolt/two/deactivate()
	. = ..()
...
/datum/discipline_power/thaumaturgy_path_of_levinbolt/three/activate()
	. = ..()

	//proc for counterattack
	RegisterSignal(owner, COMSIG_PARENT_ATTACKBY, PROC_REF(power_array_counter))
...
/datum/discipline_power/thaumaturgy_path_of_levinbolt/three/deactivate()
	. = ..()
	UnregisterSignal(owner, COMSIG_PARENT_ATTACKBY)
...
/datum/discipline_power/thaumaturgy_path_of_levinbolt/three/proc/power_array_counter(mob/source, obj/item/weapon, mob/living/attacker)
	SIGNAL_HANDLER
...
/datum/discipline_power/thaumaturgy_path_of_levinbolt/four
	name = "Zeus' Fury"
...
/datum/discipline_power/thaumaturgy_path_of_levinbolt/four/activate(mob/living/target)
	. = ..()
...
/datum/discipline_power/thaumaturgy_path_of_levinbolt/four/proc/execute_zeus_fury(mob/living/primary_target)
	owner.cut_overlay(electric_halo)

	var/max_bounces = success_roll // Lightning chain dependent upon successes - two successes, two targets hit
	var/bolt_damage = 20 + (success_roll * 4) // Base 20 + 4 per success
	...
	playsound(get_turf(owner), 'sound/magic/lightningbolt.ogg', min(50 + (success_roll * 10), 100), TRUE, extrarange = success_roll)

	// bolt of lightning to the first target
	owner.Beam(primary_target, icon_state="lightning[rand(1,12)]", time = (5 + success_roll))
...
/datum/discipline_power/thaumaturgy_path_of_levinbolt/four/proc/chain_bolt(atom/origin, mob/living/current_target, bolt_energy, bounces_left, list/already_hit)
	...
	current_target.Jitter(20 + (success_roll * 5))
	if(ishuman(current_target))
		var/mob/living/carbon/human/H = current_target
		H.electrocution_animation(40 + (success_roll * 10))
	...
	if(next_target)
		var/chain_delay = max(5 - success_roll, 1) // slight delay for coolness
...
/datum/discipline_power/thaumaturgy_path_of_levinbolt/four/proc/continue_chain(atom/origin, mob/living/next_target, bolt_energy, bounces_left, list/already_hit)
	origin.Beam(next_target, icon_state="lightning[rand(1,12)]", time = (5 + success_roll))
	// With more successes, damage loss per bounce is reduced
	var/energy_retention = 0.8 + (success_roll * 0.05) // 80% base, up to 100% with 5 successes
...
/datum/discipline_power/thaumaturgy_path_of_levinbolt/four/proc/cancel_fury(reason)
	if(electric_halo)
		owner.cut_overlay(electric_halo)
...
/datum/discipline_power/thaumaturgy_path_of_levinbolt/five/activate(atom/target)
	. = ..()
	...
	RegisterSignal(owner, COMSIG_PARENT_ATTACKBY, PROC_REF(storm_counter))
...
/datum/discipline_power/thaumaturgy_path_of_levinbolt/five/proc/create_sparks()
	if(!owner)
		return
...
/datum/discipline_power/thaumaturgy_path_of_levinbolt/five/proc/fire_lightning_bolt()
	if(!owner)
		return
...
/datum/discipline_power/thaumaturgy_path_of_levinbolt/five/proc/storm_counter(mob/source, obj/item/weapon, mob/living/attacker)
	SIGNAL_HANDLER
...
/datum/discipline_power/thaumaturgy_path_of_levinbolt/five/proc/storm_target_click(mob/source, atom/target, params)
	SIGNAL_HANDLER

	return levinbolt_target_click(source, target, params, TRUE)

/datum/discipline_power/thaumaturgy_path_of_levinbolt/five/deactivate()
	if(!owner)
		return

	UnregisterSignal(owner, list(COMSIG_CLICK, COMSIG_PARENT_ATTACKBY))
...
/datum/discipline_power/thaumaturgy_path_of_levinbolt/five/Destroy()
	if(spark_timer)
		deltimer(spark_timer)
	if(lightning_timer)
		deltimer(lightning_timer)
