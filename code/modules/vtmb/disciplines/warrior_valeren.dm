...
/datum/discipline_power/valeren_warrior/armor_of_caines_fury/activate()
	...
	totaldice = (get_a_strength(owner) + fortitudelevel)
	var/mypower = secret_vampireroll(totaldice, 8)
...
/datum/discipline_power/valeren_warrior/samiels_vengeance/activate(mob/target)
	...
	if(!I)
		...
		owner.dna.species.harm(owner, target, null, TRUE)
		if(!ishuman(target))
		...
		target.attacked_by(I, owner, TRUE)
