...
/obj/abyssrune/comforting_darkness/complete()
	...
	var/dice = (get_a_stamina(invoker)+get_a_occult(invoker))
	...
	if(ckey in roll_cache)
		...
		roll = secret_vampireroll(dice, 8, invoker)
...
/obj/abyssrune/reflections_of_hollow_revelation/complete()
	...
	var/mypower = (get_a_perception(user)+get_a_occult(user))
	var/roll_result = secret_vampireroll(mypower, 7, user)
	if(roll_result >= 3)
...
/obj/abyssrune/reflections_of_hollow_revelation/proc/scry_target(mob/living/carbon/human/target, mob/user)
	...
	if(iskindred(target))
		...
		if(vampire && (vampire.get_discipline("Obtenebration") || vampire.get_discipline("Auspex")))
			var/theirpower = (get_a_perception(target)+get_a_occult(target)) // Mentality + Social in place of Perception + Occult
			if(secret_vampireroll(theirpower, 8, target, TRUE) >= 3)
