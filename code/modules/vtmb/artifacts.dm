...
/obj/item/vtm_artifact/attack_self(mob/living/user)
	...
	if(do_mob(user, src, 15 SECONDS))
		var/result1 = secret_vampireroll(get_a_occult(user)+get_a_investigation(user), 7, user)
		switch(result1)
			if(-1)
				to_chat(user, "<span class='warning'>[src] внезапно обратился в прах!</span>")
				src.Destroy()
			if(1 to 2)
				...
				spawn(2 SECONDS)
					...
					switch(result2)
						if(-1)
							...
							to_chat(user, "<span class='warning'>Артефакт рассыпался в прах, ты чувствуешь некое беспокойство. Твой разум был не готов...</span>")
							src.Destroy()
							spawn( 10 SECONDS)
								switch(rand(1,4))
									if(1)
						...
						if(0 to 1)
							to_chat(user, "<span class='warning'>Кажется это действительно просто пустышка с барахолки...</span>")
						if(2)
							switch(rand(1,3))
								if(1)
									to_chat(user, "<span class='warning'>Ты стал на шаг ближе к разгадке этого артефакта...</span>")
								if(2)
									to_chat(user, "<span class='warning'>Этот предмет тебе кажется знакомым...</span>")

								if(3)
									to_chat(user, "<span class='warning'>Ты слышишь чей-то шепот...</span>")
									playsound(get_turf(user), 'code/modules/wod13/sounds/CURSE.ogg', 30, TRUE)
