	...
	if(stat < 1)
		for(var/mob/living/carbon/human/H in oviewers(7, src))
			if(H)
				...
				if(H.masquerade < 3)
					Aggro(H, FALSE)
		for(var/mob/living/carbon/werewolf/lupus/W in oviewers(7, src))
			if(W)
				if((W.hispo || !HAS_TRAIT(W, TRAIT_DOGWOLF)))
					Aggro(W, FALSE)
		for(var/mob/living/carbon/werewolf/crinos/C in oviewers(7, src))
			if(C)
				Aggro(C, FALSE)
