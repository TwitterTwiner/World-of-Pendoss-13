...
/datum/action/vicissitude_blood/Trigger()
	...
	var/mob/living/simple_animal/hostile/bloodcrawler/ZOV
	var/mob/living/simple_animal/hostile/tzimisce_beast/TB = owner
	ZOV.bloodpool = NG.bloodpool
	...
	if(NG.bloodpool < 2)
		...
		return
	NG.bloodpool = max(0, NG.bloodpool-2)
	var/datum/warform/Warform = new
	if(owner == /mob/living/simple_animal/hostile/tzimisce_beast)
		Warform.transform(/mob/living/simple_animal/hostile/bloodcrawler, TB, TRUE)
	else
...
/datum/action/vicissitude_form/Trigger()
	...
	if(NG.bloodpool < 3)
		to_chat(owner, "<span class='warning'>You don't have enough <b>BLOOD</b> to do that!</span>")
		return
	NG.bloodpool = max(0, NG.bloodpool-3)
	var/datum/warform/Warform = new
