...
/proc/AnnounceArrival(mob/living/carbon/human/character, rank)
	...
	deadchat_broadcast("<span class='game'> has arrived at the city at <span class='name'>[A.name]</span>.</span>", "<span class='game'><span class='name'>[character.real_name]</span> ([rank])</span>", follow_target = character, message_type=DEADCHAT_ARRIVALRATTLE)
...
//		displayed_rank = character.client.prefs.alt_titles_preferences[rank]
	...
	announcer.announce("ARRIVAL", character.real_name, rank, list()) //make the list empty to make it announce it in common//make the list empty to make it announce it in common
